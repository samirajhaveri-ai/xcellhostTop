  ...
  template: `
    <section class="platform-strip" aria-labelledby="platform-strip-title">
      <div class="platform-strip-inner">
        <h2 id="platform-strip-title"><span>We are</span>Platform agnostic</h2>
          ...
          @for (group of groups; track group.label) {
            <ul class="platform-group" [attr.aria-label]="group.label">
              @for (logo of group.logos; track logo.file) {
                <li [title]="logo.name"><img [src]="'/assets/images/platforms/' + logo.file" [alt]="logo.name" width="32" height="32" loading="lazy" decoding="async" /></li>
              }
            </ul>
          }
        </div>
...
export class PlatformStripComponent {
  readonly groups = [
    { label: 'Cloud platforms', logos: [{ name: 'AWS', file: 'aws.svg' }, { name: 'Microsoft Azure', file: 'azure.svg' }, { name: 'Google Cloud', file: 'gcp.png' }] },
