...
@Component({
  selector: 'xh-data-resilience-source',
  standalone: true,
  template: `
    <iframe
      #frame
      [src]="view === 'hero' ? heroUrl : detailsUrl"
      [title]="view === 'hero' ? 'Data Resilience introduction' : 'Data Resilience details and frequently asked questions'"
      scrolling="no"
      (load)="onLoad()"
    ></iframe>
  `,
...
export class DataResilienceSourceComponent implements OnDestroy {
  @Input({ required: true }) view: 'hero' | 'details' = 'details';
  @ViewChild('frame') private frame?: ElementRef<HTMLIFrameElement>;

  private readonly sanitizer = inject(DomSanitizer);
  readonly heroUrl = this.sanitizer.bypassSecurityTrustResourceUrl('/data-resilience-content.html?view=hero');
  readonly detailsUrl = this.sanitizer.bypassSecurityTrustResourceUrl('/data-resilience-content.html?view=details');
